...
import { getTopicLikes } from '@/app/actions';
...
export async function getTopicsWithLikes(): Promise<Topic[]> {
  const topics: Topic[] = [
    {
      id: 1,
  ...
  // Fetch like counts for all topics
  const topicsWithLikes = await Promise.all(
    topics.map(async topic => {
      const likeCount = await getTopicLikes(topic.id);
      return { ...topic, likeCount };
    })
  );

  return topicsWithLikes;
}
